...
$stats = $pdo->prepare("SELECT 
    ...
    FROM patients WHERE DATE(created_at) = ?")->execute([$date, $date, $date, $date])->fetch();

// Get patients for the day
$patients = $pdo->prepare("SELECT * FROM patients WHERE DATE(created_at) = ? ORDER BY created_at")
               ->execute([$date])
               ->fetchAll();
...
foreach ($patients as $index => $patient) {
    echo ($index + 1) . "\t";
    echo htmlspecialchars($patient['first_name'] . ' ' . htmlspecialchars($patient['last_name'])) . "\t";
